...
// Configuration structure for flexible paths and patterns
#[derive(Debug, Clone)]
struct Config {
    rom_dir: String,
    logs_dir: String,
...
#[derive(Debug, Clone)]
struct SystemPatterns {
    bracket_patterns: Vec<String>,
        ...
        Config {
            ...
            system_patterns: SystemPatterns {
                ...
                contains_patterns: vec![
                    "system menu", "system settings", "system update",
                    "dashboard", "kernel", "bootloader",
                    "nand dump", "nand backup", "flash dump",
                    "devkit", "dev kit", "kiosk",
                    "ipl rom", "boot.rom", "iplrom",
                ].into_iter().map(String::from).collect(),
            },
            stop_words: vec![
...
// 2. System/BIOS files → always use folders, even if single ROM
// 3. Single ROM games where ROM name matches game name → directly in roms/[ROM file]
// 4. Single ROM games where ROM name differs from game name → use folder (roms/[Game Name]/[ROM file])
//    Example: Game "Memory (Japan)", ROM "MEMORY.ASF" → roms/Memory (Japan)/MEMORY.ASF
// 5. ROMs with internal paths (containing \ or /) → preserve folder structure
...
impl RomAuditor {
        ...
        for file in files {
            ...
            if !matching_entries.is_empty() {
                ...
                for rom_entry in &matching_entries {
                    ...
                    // Use game folder for:
                    // 1. System files (always)
                    // 2. Games with multiple ROMs
                    // 3. Single ROM games where name doesn't match
                    ...
                    // 4. ROMs with internal folder structure
    ...
    fn is_system_file(&self, game_name: &str) -> bool {
        let name_lower = game_name.to_lowercase();
        
        // Check against configured patterns
        self.config.system_patterns.bracket_patterns.iter()
            .any(|pattern| name_lower.contains(pattern)) ||
        self.config.system_patterns.prefix_patterns.iter()
            .any(|pattern| name_lower.starts_with(pattern)) ||
        self.config.system_patterns.contains_patterns.iter()
            .any(|pattern| name_lower.contains(pattern))
    }
    ...
    fn identify_games_needing_folders(&self) -> HashSet<String> {
        ...
        for game_name in &self.all_games {
            ...
            // Three cases for needing folders:
            // 1. System/BIOS files always get folders
            // 2. Games with multiple ROMs always get folders
            // 3. Single ROM games where ROM name doesn't match game name
            ...
            if self.is_system_file(game_name) || actual_rom_count > 1 {
    ...
    fn write_logs(&self, results: &ScanResult) -> Result<(), RomAuditError> {
        ...
        if !games_needing_folders.is_empty() {
            ...
            writeln!(folders_file, "- System/BIOS files (always)")?;
